...
def to_snake(name: str) -> str:
    """CourseReview -> course_review ; Doctor -> doctor"""
    if not name:
        return name
    return re.sub(r"(?<!^)(?=[A-Z])", "_", name).lower()


def _unwrap_type(t: str) -> str:
    """Optional[float] -> float ; List[int] -> int ; float -> float"""
    if not t:
        return ""
    m = re.match(r"^(?:Optional|List)\[(.+)\]$", t.strip())
...
# valeurs pour les tests d'update
_UPDATED_VALUES = {
    "int": "2",
    "float": "2.5",
    "bool": "False",
...
class TestGenerator:
    ...
    def _gen_seed_helper(self, entity: str) -> str:
        ...
        for f in self._model_fields(entity):
            ...
            if fk:
                parent = fk.split(".")[0]
                var = name[:-3] if name.endswith("_id") else to_snake(parent)
                parent_lines.append(f"    {var} = _create_{to_snake(parent)}(db)")
        ...
        return (
            f"def _create_{to_snake(entity)}(db):\n"
            f"{body}"
    ...
    def _classify(self, fn: dict) -> Optional[str]:
        ...
        if method == "GET":
            if "{" not in path:
                return "list"
            # dernier segment = un paramètre -> get_by_id, sinon relationnel
            last = path.rstrip("/").split("/")[-1]
            return "get_by_id" if last.startswith("{") and last.endswith("}") else "relational"
    ...
    # ---------------- url helper ----------------
    @staticmethod
    def _sub_param(path: str, replacement: str) -> str:
        """Remplace n'importe quel {xxx} par `replacement`."""
    ...
    def _build_payload(self, entity: str):
        ...
        for f in self._create_schema_fields(entity):
            ...
            if name in fk_map:
                parent = fk_map[name]
                var = name[:-3] if name.endswith("_id") else to_snake(parent)
                seed_lines.append(f"    {var} = _create_{to_snake(parent)}(db)")
                capture_lines.append(f"    {name}_value = {var}.id")
                items.append(f'"{name}": {name}_value')
            else:
                ftype = _unwrap_type(f.get("type"))
                items.append(f'"{name}": {_PY_VALUES.get(ftype, chr(34) + "test_string" + chr(34))}')
    ...
    def _gen_create(self, entity: str, fn: dict) -> str:
        ...
        for f in self._create_schema_fields(entity):
            if f["name"] in fk_names:
                continue
            val = _PY_VALUES.get(_unwrap_type(f.get("type")), '"test_string"')
    ...
    def _gen_get_by_id(self, entity: str, fn: dict) -> str:
        op, path = fn["operationId"], fn["path"]
        ok_url = self._sub_param(path, "{obj_id}")
        nf_url = self._sub_param(path, "999999")
        return "\n".join([
            f"def test_{op}():",
            "    db = TestingSessionLocal()",
            f"    obj = _create_{to_snake(entity)}(db)",
            "    obj_id = obj.id",
            "    db.close()",
    ...
    def _gen_relational(self, entity: str, fn: dict) -> str:
        ...
        for fname, parent in fks:
            if to_snake(parent) in first_seg or parent.lower() in first_seg:
                fk_field = fname
                break
        url = self._sub_param(path, "{parent_id}")
        return "\n".join([
            f"def test_{op}():",
            "    db = TestingSessionLocal()",
            f"    obj = _create_{to_snake(entity)}(db)",
    ...
    def _gen_update(self, entity: str, fn: dict) -> str:
        ...
        for f in self._create_schema_fields(entity):
            ...
            if name in fk_names:
                ...
                val = _UPDATED_VALUES.get(ftype, '"updated_string"')
    ...
    def _gen_header(self, entity: str) -> str:
        model_imports = "\n".join(
            f"from app.models.{to_snake(e)} import {e}" for e in self._seed_order(entity)
        )
        return (
            "import pytest\n"
            ...
            f"from app.routes.{to_snake(entity)} import router\n"
            f"{model_imports}\n\n"
